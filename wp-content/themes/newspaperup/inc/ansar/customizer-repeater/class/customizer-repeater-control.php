...
class Newspaperup_Repeater_Control extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'newspaperup' );
		if ( ! empty( $args['add_field_label'] ) ) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__( 'Customizer Repeater', 'newspaperup' );
	...
	public function render_content() {
		...
		<input type="hidden" value="<?php echo count( $json ); ?>" id="newspaperup_social_limit"/>
					...
					<div class="customizer-repeater-box-content-hidden">
						...
						if($this->customizer_repeater_test_title_control==true){
							$this->input_control(array(
								...
								'type'  => apply_filters('newspaperup_repeater_input_types_filter', '', $this->id, 'customizer_repeater_test_title_control' ),
							), $test_title);
						}
						...
						if($this->customizer_repeater_title_control==true){
							$this->input_control(array(
                                ...
                                'type'  => apply_filters('newspaperup_repeater_input_types_filter', '', $this->id, 'customizer_repeater_title_control' ),
							), $title);
						}
						if($this->customizer_repeater_subtitle_control==true){
							$this->input_control(array(
								'class' => 'customizer-repeater-subtitle-control',
								'type'  => apply_filters('newspaperup_repeater_input_types_filter', '', $this->id, 'customizer_repeater_subtitle_control' ),
							), $subtitle);
						}
						
						if($this->customizer_repeater_link_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Link','newspaperup' ), $this->id, 'customizer_repeater_link_control' ),
								'class' => 'customizer-repeater-link-control',
								'sanitize_callback' => 'esc_url_raw',
                                'type'  => apply_filters('newspaperup_repeater_input_types_filter', '', $this->id, 'customizer_repeater_link_control' ),
							), $link);
						}
						...
						<button type="button" class="social-repeater-general-control-remove-field" <?php if ( $it == 0 ) {
							...
							<?php esc_html_e( 'Delete field', 'newspaperup' ); ?>
						</button>
			...
			<div class="customizer-repeater-general-control-repeater-container">
				...
				<div class="customizer-repeater-box-content-hidden">
					...
					if($this->customizer_repeater_color_control==true){
						$this->input_control(array(
							'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Color','newspaperup' ), $this->id, 'customizer_repeater_color_control' ),
							'class' => 'customizer-repeater-color-control',
							'type'  => apply_filters('newspaperup_repeater_input_types_filter', 'color', $this->id, 'customizer_repeater_color_control' ),
							'sanitize_callback' => 'sanitize_hex_color'
						) );
					}
					if ( $this->customizer_repeater_title_control == true ) {
						$this->input_control( array(
							'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Title','newspaperup' ), $this->id, 'customizer_repeater_title_control' ),
							'class' => 'customizer-repeater-title-control',
                            'type'  => apply_filters('newspaperup_repeater_input_types_filter', '', $this->id, 'customizer_repeater_title_control' ),
						) );
					}
					...
					if ( $this->customizer_repeater_text_control == true ) {
						$this->input_control( array(
							...
							'type'  => apply_filters('newspaperup_repeater_input_types_filter', 'textarea', $this->id, 'customizer_repeater_text_control' ),
						) );
					}
...
//print_r($options);
	...
	<?php esc_html_e('Open link in a new tab','newspaperup'); ?>
	<span class="switch">
	...
	private function icon_picker_control($value = '', $show = ''){ ?>
		<div class="social-repeater-general-control-icon" <?php if( $show === 'customizer_repeater_image' || $show === 'customizer_repeater_none' ) { echo 'style="display:none;"'; } ?>>
            <span class="customize-control-title">
                <?php esc_html_e('Icon','newspaperup'); ?>
            </span>
			<span class="description customize-control-description">
                <?php
                echo sprintf(
	                esc_html__( 'Note: Some icons may not be displayed here. You can see the full list of icons at %1$s.', 'newspaperup' ),
	...
	private function repeater_control($value = ''){
		...
		$show_del        = 0; ?>
		<span class="customize-control-title"><?php esc_html_e( 'Social icons', 'newspaperup' ); ?></span>
		<?php
		if(!empty($value)) {
			...
			<div class="customizer-repeater-social-repeater">
				<div class="customizer-repeater-social-repeater-container">
					...
					<input type="text" class="customizer-repeater-social-repeater-link"
					       placeholder="<?php esc_attr_e( 'Link', 'newspaperup' ); ?>">
					<input type="hidden" class="customizer-repeater-social-repeater-id" value="">
					<button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'newspaperup' ); ?>
					</button>
				</div>
				<input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
			</div>
			<button class="social-repeater-add-social-item button-secondary"><?php esc_html_e( 'Add Icon', 'newspaperup' ); ?></button>
			<?php
		} else { ?>
			<div class="customizer-repeater-social-repeater">
				<?php
					...
					<div class="customizer-repeater-social-repeater-container">
						...
						<button class="social-repeater-remove-social-item"
						        style="<?php if ( $show_del == 1 ) {
							        echo "display:none";
						        } ?>"><?php esc_html_e( 'Remove Icon', 'newspaperup' ); ?></button>
					</div>
					<?php
				} ?>
